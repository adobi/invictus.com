...
<?php endif ?>
    ...
    <?php echo form_open(base_url().'cig/index/' . ($_POST && ($controller || $model || $view) ? 'generate' : ''), array('id'=>'edit-form', 'class'=>'form-horizontal')) ?>    
        ...
        <fieldset class="form-actions">
            <button class="btn btn-primary"><i class="icon-search icon-white"></i>View code</button> 
            <?php if (!$_POST || validation_errors()): ?>
            <?php else: ?>
                &nbsp;<button class="btn btn-primary"><i class="icon-time icon-white"></i>Generate code</button> 
            <?php endif ?>
