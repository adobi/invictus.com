...
<?php echo form_open('', array('id'=>'edit-form', 'data-ajax-form'=>1)) ?>    
    ...
    <div class="right-side-scroll">
      ...
      <fieldset class="control-group">
          ...
          <div class="controls">
              <input type="text" name = "logo" id = "logo" class = "span4" value = "<?php echo base_url() . 'uploads/original/'.$item->logo ?>"/>
              <br>
              <img src="<?php echo base_url() . 'uploads/original/'.$item->logo ?>" alt="" style="width:64px">
          </div>
      </fieldset>       
      <fieldset class="control-group">
          <label class="control-label" for="link_text">Videos</label>
          <div class="controls">
            <?php if ($videos): ?>
              <ul class="thumbnails">
                <?php foreach ($videos as $p): ?>
                  <li class="thumbnail">
                    ...
                    <p style="text-align:center">
                      <input type="radio" name="video_id" value="<?php echo $p->id ?>">
                    </p>
                  </li>
                <?php endforeach ?>
              </ul>
      ...
      <?php if ($item->logo): ?>
        <fieldset class="form-actions right">
            <button class="btn btn-primary" rel="tooltip" title="Save as pressrelease" data-noaction="1"><i class="icon-ok icon-white"></i> Save as press release</button>
        </fieldset>       
      <?php else: ?>
        <div class="alert alert-error">
